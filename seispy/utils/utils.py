...
def fftgram(ts, stride):
    ...
    out = Spectrogram(dat,
                      ...
                      dt=dt, copy=False,
                      unit=ts.unit * (u.Hz**-0.5), dtype=dtype)
    ...
    for station in dict1.keys():
        ...
        for channel in dict1[station].keys():
            ...
            if np.abs(end_diff) > 0:
                end_zeros = np.zeros(np.abs(end_diff) *
                                     tr1.sample_rate.value - 1)
...
def gaussian_noise(psd_amp, sample_rate, duration, segdur=None, name=None):
    ...
    for seg in range(Nsegs):
        ...
        if seg == 0:
            ts =\
                np.real(np.fft.ifft(np.concatenate(
                    (np.zeros(1), spec, np.zeros(1), np.flipud(spec) * nsamps))))
        else:
            ts = np.hstack((ts, np.real(np.fft.ifft(np.concatenate(
                (np.zeros(1), spec, np.zeros(1), np.flipud(spec) * nsamps))))))
...
def get_homestake_channels(type):
    ...
    status_chan = ['LCE', 'LCQ', 'LPL', 'LCL', 'LCC', 'QBD', 'QBP', 'QDG', 'QDL',
                   ...
                   'VM2', 'VM3', 'VPB', 'VTW']
    useful_chan = ['HHE', 'HHN', 'HHZ', 'LCE',
                   'LCQ', 'VEP', 'VKI', 'VM1', 'VM2', 'VM3']
    ...
    if (type.lower() == 'all'):
        ...
        raise ValueError(
            'type should be \'all\' or \'data\' or \'status\' or \'useful\'.')
