...
class TestMass:
    def __init__(self,name,position,mass,freqs,is_coherent=True):
        """
        Constructor
        position -- 3d position vector x,y,z (z<0==>underground). Assume plane 
            surface at =0
        mass -- kg
    ...
    def get_acceleration_R_surface(self,freq,theta,phi,a,seismic_field):
        ...
        omega = speed * kmag #### OMEGA=speed*kmag or speed*krhomag??????
    ...
    def get_acceleration_R_cavity(self,freq,theta,phi,a,seismic_field):
        ...
        AV1=V1*a 
        AV2=V2*a  
        omega = speed * kmag ##### kmag or krhomag?????
        ...
        ax = del5 * 1j * (AH1 * np.exp(-h1*h) + AH2 * np.exp(-h2*h)) * k[0] / kmag
    ...
    def get_acceleration_P_cavity(self,freq,theta,phi,a,seismic_field):
        ...
        dela=self.get_acceleration_P_surface(freq,theta,phi,a,seismic_field)
    ...
    def get_acceleration_SV_cavity(self,freq,theta,phi,a,seismic_field):
        ...
        dela=self.get_acceleration_SV_surface(freq,theta,phi,a,seismic_field)
        ...
        pvec=k/kmag
        shvec=np.array([-k[2],k[1],0])    #dispacement vector
        shvec=shvec/np.linalg.norm(shvec)
        nhat=np.cross(pvec,shvec)
        #nhat=nhat/np.linalg.norm(nhat) ## WHY SHOULDNT I HAVE TO NORMALIZE???
